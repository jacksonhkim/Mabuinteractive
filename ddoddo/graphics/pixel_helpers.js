...
// ── 캐릭터 애니메이션 스프라이트 그리기 (프레임 단위 커팅) ──
export const drawPlayerSprite = (ctx, img, x, y, w, h, totalFrames, currentFrameIndex, flip = false) => {
    if (!img || !img.complete || img.naturalWidth === 0) return false;

    // Each frame width inside the sprite sheet
    const sourceFrameW = img.naturalWidth / totalFrames;
    const sourceFrameH = img.naturalHeight;
    const sx = currentFrameIndex * sourceFrameW;
    const sy = 0;
    ...
    if (flip) {
        ctx.translate(Math.floor(x + w), Math.floor(y));
        ctx.scale(-1, 1);
        ctx.drawImage(img, sx, sy, sourceFrameW, sourceFrameH, 0, 0, Math.floor(w), Math.floor(h));
    } else {
        ctx.drawImage(img, sx, sy, sourceFrameW, sourceFrameH, Math.floor(x), Math.floor(y), Math.floor(w), Math.floor(h));
    }
    ctx.restore();
    return true;
};
