...
import { PLAYER_IMAGES, PLAYER_SPRITES } from '../data/assets.js';
import { drawRect, drawCharImg, drawPlayerSprite, drawPixelGrid } from './pixel_helpers.js';
...
export function drawPixelTotoV5(ctx, x, y, w, h, vy = 0, playerState = null) {
    // If no state provided (e.g. world map or title screen), fallback to simple idle animation
    let imgToDraw = PLAYER_SPRITES.ch_player1.idle;
    let frames = 4;
    let frameIdx = Math.floor((Date.now() / 150) % frames);

    if (playerState) {
        if (playerState.invincibleTime > 60) {
            // Hit / damage priority (simple approximation: early invincibility frames)
            imgToDraw = PLAYER_SPRITES.ch_player1.hit;
            frames = 3;
            frameIdx = Math.floor((Date.now() / 100) % frames);
        } else if (playerState.chargeValue > 0) {
            imgToDraw = PLAYER_SPRITES.ch_player1.charge;
            frames = 6;
            frameIdx = Math.floor((playerState.chargeValue / playerState.maxChargeValue) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        } else if (Date.now() - playerState.lastShotTime < 200) {
            imgToDraw = PLAYER_SPRITES.ch_player1.attack;
            frames = 4;
            frameIdx = Math.floor(((Date.now() - playerState.lastShotTime) / 200) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        }
    }

    if (drawPlayerSprite(ctx, imgToDraw, x, y, w, h, frames, frameIdx, false)) return;
...
// ── 루루 (나비 - 스피드 타입) V5 ──
export function drawPixelLuluV2(ctx, x, y, w, h, vy = 0, playerState = null) {
    let imgToDraw = PLAYER_SPRITES.ch_player2.idle;
    let frames = 4;
    let frameIdx = Math.floor((Date.now() / 150) % frames);

    if (playerState) {
        if (playerState.invincibleTime > 60) {
            imgToDraw = PLAYER_SPRITES.ch_player2.hit;
            frames = 3;
            frameIdx = Math.floor((Date.now() / 100) % frames);
        } else if (playerState.chargeValue > 0) {
            imgToDraw = PLAYER_SPRITES.ch_player2.charge;
            frames = 6;
            frameIdx = Math.floor((playerState.chargeValue / playerState.maxChargeValue) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        } else if (Date.now() - playerState.lastShotTime < 200) {
            imgToDraw = PLAYER_SPRITES.ch_player2.attack;
            frames = 4;
            frameIdx = Math.floor(((Date.now() - playerState.lastShotTime) / 200) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        }
...
// ── 카카 (딱정벌레 - 파워 타입) V5 ──
export function drawPixelKakaV2(ctx, x, y, w, h, vy = 0, playerState = null) {
    let imgToDraw = PLAYER_SPRITES.ch_player3.idle;
    let frames = 4;
    let frameIdx = Math.floor((Date.now() / 150) % frames);

    if (playerState) {
        if (playerState.invincibleTime > 60) {
            imgToDraw = PLAYER_SPRITES.ch_player3.hit;
            frames = 3;
            frameIdx = Math.floor((Date.now() / 100) % frames);
        } else if (playerState.chargeValue > 0) {
            imgToDraw = PLAYER_SPRITES.ch_player3.charge;
            frames = 6;
            frameIdx = Math.floor((playerState.chargeValue / playerState.maxChargeValue) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        } else if (Date.now() - playerState.lastShotTime < 200) {
            imgToDraw = PLAYER_SPRITES.ch_player3.attack;
            frames = 4;
            frameIdx = Math.floor(((Date.now() - playerState.lastShotTime) / 200) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        }
...
// ── 모모 (무당벌레 - 방어 타입) V5 ──
export function drawPixelMomoV2(ctx, x, y, w, h, vy = 0, playerState = null) {
    let imgToDraw = PLAYER_SPRITES.ch_player4.idle;
    let frames = 4;
    let frameIdx = Math.floor((Date.now() / 150) % frames);

    if (playerState) {
        if (playerState.invincibleTime > 60) {
            imgToDraw = PLAYER_SPRITES.ch_player4.hit;
            frames = 3;
            frameIdx = Math.floor((Date.now() / 100) % frames);
        } else if (playerState.chargeValue > 0) {
            imgToDraw = PLAYER_SPRITES.ch_player4.charge;
            frames = 6;
            frameIdx = Math.floor((playerState.chargeValue / playerState.maxChargeValue) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        } else if (Date.now() - playerState.lastShotTime < 200) {
            imgToDraw = PLAYER_SPRITES.ch_player4.attack;
            frames = 4;
            frameIdx = Math.floor(((Date.now() - playerState.lastShotTime) / 200) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        }
...
// ── 피피 (잠자리 - 특수 타입) V5 ──
export function drawPixelPipiV2(ctx, x, y, w, h, vy = 0, playerState = null) {
    let imgToDraw = PLAYER_SPRITES.ch_player5.idle;
    let frames = 4;
    let frameIdx = Math.floor((Date.now() / 150) % frames);

    if (playerState) {
        if (playerState.invincibleTime > 60) {
            imgToDraw = PLAYER_SPRITES.ch_player5.hit;
            frames = 3;
            frameIdx = Math.floor((Date.now() / 100) % frames);
        } else if (playerState.chargeValue > 0) {
            imgToDraw = PLAYER_SPRITES.ch_player5.charge;
            frames = 6;
            frameIdx = Math.floor((playerState.chargeValue / playerState.maxChargeValue) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        } else if (Date.now() - playerState.lastShotTime < 200) {
            imgToDraw = PLAYER_SPRITES.ch_player5.attack;
            frames = 4;
            frameIdx = Math.floor(((Date.now() - playerState.lastShotTime) / 200) * frames);
            if (frameIdx >= frames) frameIdx = frames - 1;
        }
