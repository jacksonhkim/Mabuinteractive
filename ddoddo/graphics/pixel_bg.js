...
export function drawPixelForestV2(ctx, CONFIG, state) {
    ...
    const envStyles = {
        ...
        10: { // 고딕 성당 (Stage 10)
            skyTop: '#0a0a0f', skyBot: '#1a1a2e', sunColor: 'rgba(200, 220, 255, 0.15)',
            treeColor: (c) => `rgba(20, 20, 30, ${c})`, groundColor: '#0c0c14',
            detailColor: '#16213e', accentColor: '#4ecca3', godRays: true
        }
    };
    ...
    if (stage === 1) {
        ...
        let layersDrawn = false;
        if (drawLayer(BACKGROUND_IMAGES.s8_layer01, 0.2)) layersDrawn = true;
        if (drawLayer(BACKGROUND_IMAGES.s8_layer02, 0.5)) layersDrawn = true;
        if (drawLayer(BACKGROUND_IMAGES.s8_layer03, 0.5)) layersDrawn = true; // layer02와 동일 속도
        if (layersDrawn) return;
    } else if (stage === 9) {
        let layersDrawn = false;
        ...
        if (drawLayer(BACKGROUND_IMAGES.s9_layer04, 0)) layersDrawn = true;
        if (drawLayer(BACKGROUND_IMAGES.s9_layer05, 1.0)) layersDrawn = true;
        if (layersDrawn) return;
    } else if (stage === 10) {
        let layersDrawn = false;
        // Stage 10: Cathedral (Windows, Interior, Particles)
        if (drawLayer(BACKGROUND_IMAGES.s10_layer01, 0.2)) layersDrawn = true; // Far windows (Synchronized)
        if (drawLayer(BACKGROUND_IMAGES.s10_layer02, 0.2)) layersDrawn = true; // Middle pillars/floor (Synchronized)
        if (drawLayer(BACKGROUND_IMAGES.s10_layer03, 0.8)) layersDrawn = true; // Near details
        if (layersDrawn) return;
    }
