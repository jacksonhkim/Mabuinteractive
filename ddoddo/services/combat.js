...
export function spawnItem(x, y) {
    ...
    if (type) {
        // 아이템 스폰 시 좌/우 위/아래 초기 속도를 랜덤 부여
        const vx = -2 - Math.random() * 2;
        const vy = (Math.random() - 0.5) * 4;
        state.items.push({ x, y, type, vx, vy, time: 0, width: 52, height: 52 });
    }
}
...
export function updateItems() {
    for (let i = state.items.length - 1; i >= 0; i--) {
        ...
        item.time++;
        item.x += item.vx;
        item.y += item.vy || (Math.sin(item.time * 0.05) * 5); // 기존 startY 기준 사인파에서 직접 이동으로 변경 (혹은 bounce를 위해 vy 속성 부여)

        // 화면 위/아래 경계 튕김 (Bouncing)
        if (item.y < 0) {
            item.y = 0;
            item.vy = (item.vy || -1) * -1;
        } else if (item.y > CONFIG.SCREEN_HEIGHT - item.height) {
            item.y = CONFIG.SCREEN_HEIGHT - item.height;
            item.vy = (item.vy || 1) * -1;
        }
...
export function createExplosion(x, y, color) {
    ...
    for (let i = 0; i < 10; i++) {
        state.particles.push({
            ...
            size: Math.random() * 5 + 3, color: finalColor, alpha: 1.0
        });
    }
}
