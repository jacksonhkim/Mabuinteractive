...
import {
    ...
    startDialogue, startBossDialogue, startEndingSequence, startWorldMap
} from './ui.js';
...
function confirmCharacter() {
    ...
    state.player.width = 118;
    state.player.height = 118;
    ...
    if (isCont) {
        ...
        state.bullets = [];
        state.particles = [];
        state.keys = {}; // 잔여 키보드/터치 입력 완전 해제
        updateLivesUI();
        updateBombUI();
        sound.playConfirm();
        // 보스가 살아있으면 보스 BGM 재개, 아니면 스테이지 BGM
        if (state.boss) {
            sound.startBGM(`BOSS_${state.boss.type}`);
        } else {
...
function gameLoop() {
    try {
        if (state.isSelectingCharacter) {
            ...
            if (state.stageCleared) {
            ...
            } else if (!state.isDialogueActive && !state.isWorldMapActive && !state.isBossDialogueActive) {
                // ── 일반 게임 업데이트 ──
                updateBackground();
                updatePlayer();
                updateBullets();
                ...
                // ── 대화/월드맵 중: 무적 타이머 + 파티클만 계속 실행 ──
                // (이걸 멈추면 피격 후 invincibleTime이 카운트다운 안 돼서 영구 무적 버그 발생)
                updatePlayer();
                updateParticles();
                draw(ctx, state);
            }
